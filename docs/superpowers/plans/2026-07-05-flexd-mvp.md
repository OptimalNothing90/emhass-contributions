...
class StandingManager:
    ...
    def _save_ledger(self) -> None:
        self._ledger_path.write_text(json.dumps(self._ledger, indent=2), encoding="utf-8")
...
class Scheduler:
    def __init__(
        ...
        *,
        ...
        self._on_cycle_end = on_cycle_end  # async callback(state) for mqtt publish
    ...
    async def run_once(self, now: datetime | None = None) -> str:
        ...
        if self._on_cycle_end is not None:
            await self._on_cycle_end(result)
        return result
    ...
    def notify_change(self) -> None:
        """Debounced re-solve after a demand mutation."""
        if self._debounce_task is not None and not self._debounce_task.done():
            self._debounce_task.cancel()
        self._debounce_task = asyncio.get_event_loop().create_task(self._debounced())
...
async def run() -> None:
    ...
    async def on_cycle_end(state: str) -> None:
