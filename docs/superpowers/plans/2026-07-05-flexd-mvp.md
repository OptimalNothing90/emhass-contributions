...
- [ ] **Step 3: Add TemplateDefinition to config.py**
...
class DeadlineRule(BaseModel):
    ...
    _parse_bracket = field_validator("if_started_between", mode="before")(
        StandingDefinition._parse_window.__func__  # same "HH:MM-HH:MM" parsing, but:
    )
    # NOTE: unlike standing windows, brackets MAY wrap midnight (start >= end is legal).
    # Reuse only the regex split; skip the start<end check. Implement as its own validator:
...
(Drop the first `_parse_bracket` stub — the second explicit validator is the implementation;
shown here to make the wrap-vs-window distinction impossible to miss.)
Add `templates: list[TemplateDefinition] = []` to `FlexdConfig`.
...
class TemplateManager:
    ...
    def start(self, template_id: str, *, source: str, now: datetime) -> Demand:
        ...
        if rule is None:
            ...
            if rule.not_before is not None:
                nb = self._next_occurrence(local, _to_time(rule.not_before), same_day_ok=True)
    ...
    def _next_occurrence(self, local_now: datetime, target: time, same_day_ok: bool = True) -> datetime:
        candidate = local_now.replace(hour=target.hour, minute=target.minute, second=0, microsecond=0)
        if candidate <= local_now or not same_day_ok:
...
- [ ] **Step 5: Run template tests** — Expected: pass.

- [ ] **Step 6: Wire transports.** `create_app` gains `templates=None` param.
  - rest_api.py: `@app.post("/api/v1/templates/{template_id}/start", status_code=201)` with
    JSON body `{"source": "..."}` → `templates.start(...)`, KeyError → 404, then
    `scheduler.notify_change()`.
  - simple_api.py: `@router.post("/templates/{template_id}/start", status_code=201)` with
    `source` query param → returns `"1"`, 404 on unknown template.
  - mqtt_bridge.py `handle_message`: recognize 5-part topic
    `{base}/templates/{source}/{id}/start` (empty payload) → `templates.start(...)` +
    `notify_change`; errors to `{base}/templates/{source}/{id}/error`.
  Tests: one happy + one 404/error per transport, in `test_rest_api.py`, `test_simple_api.py`,
  `test_mqtt_bridge.py` respectively, reusing existing fixtures with a `TemplateManager` built
  from `TPL` (import from `test_templates.py`).
  Also append the commented `templates:` block (the dishwasher example from the spec) to
  `flexd.yaml.example`.

- [ ] **Step 7: Run full suite** — Expected: pass.

- [ ] **Step 8: Commit** — `feat(flexd): trigger templates with start-time deadline rules (simple/rest/mqtt intake)`
