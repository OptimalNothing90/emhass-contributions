...
| D12 | **Recipe Caveats are extracted from author's production Node-RED + EMHASS setup, months in service.** Each Caveat reflects an observed-in-prod issue or hard-won pattern, not hypothetical LLM-default. Anonymized patterns folded in: MPC watchdog with 2×/4×-cadence WARN/CRITICAL; dual-format SOC robustness (`socPct = soc <= 1 ? soc * 100 : soc`); defensive SOC fallback; audit-triplet (catch + status + format-to-JSONL); hysteresis dead-band on mode boundaries; primary + physics-fallback + EMA-auto-calibration for PV forecasts; override-layer for EMHASS edge-cases (PV-surplus override at high SOC). Specific thresholds / lat-lon / inverter specs / MQTT broker addresses / secrets / internal flow-iteration version tags stay out per D4c — only the patterns + qualitative values land. | counter-empty-section risk; signal depth not LLM-default |
